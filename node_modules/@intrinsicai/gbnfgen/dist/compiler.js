...
import { group, literal, reference, sequence, } from "./grammar.js";
import { toElementId, toListElementId, WS_REF, getDefaultGrammar, registerToGrammar, } from "./util.js";
// Turn interface properties into Grammar References
export function toGrammar(iface) {
    function propertyRules(prop) {
    ...
    const ifaceListElem = {
        ...
        alternatives: [
            ...
            literal(`[]`),
            // Non-empty list
            sequence(literal(`[`), WS_REF, reference(ifaceElem.identifier), group(sequence(literal(`,`), WS_REF, reference(ifaceElem.identifier)), "star"), literal(`]`)),
        ],
    };
    return {
        ...
        for (const member of enumNode.getMembers()) {
            ...
            if (member.isKind(ts.SyntaxKind.EnumMember)) {
                ...
                if (!initializer || !initializer.isKind(ts.SyntaxKind.StringLiteral)) {
                    throw new Error("Only string enums are supported. Please check the String enums section of the TypeScript Handbook at https://www.typescriptlang.org/docs/handbook/enums.html");
                }
...
function handleInterface(iface, declaredTypes, register) {
    // Support array versions of each of the required types as well.
    const declaredArrayTypes = new Map();
    ...
    if (iface.getTypeParameters().length > 0) {
        ...
        throw new Error(`${iface.getName()}: interfaces cannot have type parameters`);
    }
    const ifaceName = iface.getName();
    const props = [];
    for (const child of iface.getMembers()) {
        if (!child.isKind(ts.SyntaxKind.PropertySignature)) {
            throw new Error(`Invalid interface member: interfaces must only contain properties, contained ${child}`);
        }
        const propName = child.getName();
        const propType = child.getType().getText();
        // Validate one of the accepted types
        let propTypeValidated;
 ...
 * @returns
 */
export async function compile(source, rootType) {
 ...
 * @returns
 */
export function compileSync(source, rootType) {
    ...
    srcFile.forEachChild((child) => {
        switch (child.getKind()) {
            case ts.SyntaxKind.InterfaceDeclaration:
                const iface = handleInterface(child, declaredTypes, register);
            ...
            case ts.SyntaxKind.EmptyStatement:
                break;
            default:
                throw new Error(`Invalid top-level declaration of kind ${child.getKindName()}: ${child.getText()}`);
        }
    });
    // Reject when the root type is not found
    if (!declaredTypes.has(rootType)) {
        throw new Error(`Root type ${rootType} is not one of the declared types ${Array.from(declaredTypes.values())}`);
    }
