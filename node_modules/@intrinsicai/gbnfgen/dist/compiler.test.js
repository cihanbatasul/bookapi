...
test("Single interface generation", () => {
    const postalAddressGrammar = compileSync(`interface PostalAddress {
    streetNumber: number;
    street: string;
    city: string;
    state: string;
    ...
    expect(serializeGrammar(postalAddressGrammar).trimEnd()).toEqual(String.raw `
root ::= PostalAddress
...
test("Single interface with enum generation", () => {
    const postalAddressGrammar = compileSync(`enum AddressType { Business = "business", Home = "home" };
...
test("Single multiple interface and enum with references generation", () => {
  ...
  `, "Order");
    expect(serializeGrammar(resumeGrammar).trimEnd()).toEqual(String.raw `
root ::= Order
...
test("Jsonformer car example", () => {
  ...
  interface PerformanceFeature {
    ...
    expect(serializeGrammar(grammar).trimEnd()).toEqual(String.raw `
root ::= CarAndOwner
...
test("compiler errors", () => {
    ...
    `, "failure")).toThrowError(`Duplicate identifier 'name'.`);
    expect(() => compileSync(`
    interface failure {
      name1: string;
      name2: myfaketype;
    }
    `, "failure")).toThrowError(`Compilation failed: Cannot find name 'myfaketype'.`);
    expect(() => compileSync(`
      const failure = {};
    `, "failure")).toThrowError(`Invalid top-level declaration of kind VariableStatement: const failure = {};`);
    ...
    `, "failure")).toThrowError(`Invalid top-level declaration of kind TypeAliasDeclaration: type Person = string;`);
});
test("async", async () => {
  ...
  `, "failure")).rejects.toThrow("Invalid top-level declaration of kind TypeAliasDeclaration: type Person = string;");
