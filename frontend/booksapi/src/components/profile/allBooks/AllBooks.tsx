...
const AllBooks = ({ data, navigate }: Props) => {
  return (
    <div className="flex flex-col">
      ...
      <div className="flex flex-col  md:flex-col lg:flex-row gap-3 md:gap-6 lg:gap-12 flex-wrap">
        {data.AllShelves.map(
          (shelf) =>
            shelf.shelf && // Check if shelf is not null
            shelf.shelf.map((book) => (
              <div
                className="flex flex-row gap-3 h-56 sm:w-full md:w-full lg:w-[600px] cursor-pointer "
                key={book.bookID}
